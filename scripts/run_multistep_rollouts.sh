...
OUT_ROOT=outputs/multistep_rollout_official_3model_3seed
LOG_DIR=logs/multistep_rollout_official_$(date +%Y%m%d_%H%M%S)
...
run_one() {
  ...
  if [[ ! -f "$ckpt" ]]; then
    printf "%s\t%s\t%s\t%s\tmissing_ckpt\n" "$dataset" "$model" "$seed" "$steps" >> "$STATUS"
    return 0
  fi
  if [[ ! -f "$emb" ]]; then
    printf "%s\t%s\t%s\t%s\tmissing_embeddings\n" "$dataset" "$model" "$seed" "$steps" >> "$STATUS"
    return 0
  fi
...
launch_dataset "trip_known" "trip" 3 \
  "paper_gated_resfilm_3seed_trip" \
