...
We evaluate six encoders from five embedding families: Qwen3-Embedding-0.6B and Qwen3-Embedding-8B~\citep{zhang2025qwen3embedding}, bge-large-en-v1.5~\citep{xiao2024cpack}, e5-large-v2~\citep{wang2022e5}, jina-embeddings-v3~\citep{sturua2024jinaembeddingsv3}, and all-MiniLM-L6-v2~\citep{huggingfaceAllMiniLML6V2,wang2020minilm}. The original three encoders are used in the core suite, and the full set extends the analysis across additional public datasets covering procedural processes, instructional state changes, embodied household tasks, and physical commonsense stories. The resulting benchmark suite spans attribute changes, location changes, existence changes, possession, containment, heating, cooling, cleaning, opening, and object transformation.
...
Across the core datasets, one-step retrieval is substantially above chance, but the difficulty varies sharply by dataset. OpenPI-C and TRIP-known show strong transition recoverability, while ALFWorld-noGoto remains difficult despite high top-5 and top-10 retrieval. This suggests that embedding-space transitions capture coarse physical changes more reliably than fine-grained embodied object identity.
...
We presented next-state vector prediction as a way to test whether frozen text embedding models support action-conditioned state dynamics. A lightweight gated residual FiLM model trained with InfoNCE can learn meaningful transitions across procedural, instructional, embodied, and commonsense datasets. The results suggest that text embedding spaces encode useful latent structure for physical state change, but also show that exact next-state retrieval remains difficult when many candidate states are semantically close. Moving beyond next-token prediction toward next-state dynamics offers a promising route for evaluating and improving language models as representations of changing worlds.
