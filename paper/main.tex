...
\begin{abstract}
Next-token prediction has driven modern language modeling, but many reasoning problems require predicting how a world state changes after an action. We study this question directly in embedding space. Given a natural-language current state and action, we freeze a text embedding model and train only a lightweight transition probe to predict the embedding of the next state. The predicted vector is evaluated by row-level retrieval against candidate next-state embeddings. We introduce a gated residual FiLM transition model that uses the action embedding to modulate the state embedding while preserving an identity path for unchanged state features. We evaluate synthetic state changes and five public state-tracking resources: ProPara, OpenPI-C, SCONE, ALFWorld, and TRIP. Across Qwen3-Embedding, BGE, E5, Jina, and MiniLM encoders, frozen embedding spaces show substantial one-step transition structure: top-1 retrieval reaches 74.7\% on OpenPI-C and 68.2\% on TRIP known-state transitions. At the same time, ALFWorld and multi-step SCONE rollouts expose important limits. Predicted vectors often retrieve semantically similar states but miss the exact object or state instance, and autoregressive rollout rapidly accumulates error. These results suggest that current text embeddings support local action-conditioned state updates, but do not yet provide stable long-horizon world-state dynamics.
\end{abstract}
